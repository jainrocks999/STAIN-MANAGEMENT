...
import Constants from '../../component/storage';
...
function* DoDesign(action) {
  ...
  console.log('najdkaflddddddddddddddaaskfjjjjjjjjjjjjjjjjjjjjjjjjjjjjj'+JSON.stringify(p))
  if (p.Status == true) {
    yield put({
    ...
    if (p.data == '') {
      ...
      if (action.navigateTo) {
        const resetAction = StackActions.reset({
          index: 0,
          actions: [CommonActions.navigate({routeName: action.navigateTo})],
        });
        action.navigate.dispatch(resetAction);
      } else {
        const resetAction = StackActions.reset({
          index: 0,
          actions: [CommonActions.navigate({routeName: 'more'})],
        });
